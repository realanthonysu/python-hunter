...
| **cli** | Command-line tool | Click entry point, `[project.scripts]`, distributable via PyPI |
...
cp -r vibe-coding-scaffold .claude/skills/
```
...
└── references/
    ...
    └── cli.md                  # CLI tool project templates
```
...
| Skill | Relationship |
|-------|-------------|
| [vibe-coding-guardian](https://github.com/realanthonysu/vibe-coding-guardian) | Core engineering discipline — defines *what* to verify |
| [vibe-coding-coach](../../skills/vibe-coding-coach/SKILL.md) | Python edition — defines *how* to verify in Python |

`scaffold` builds the house. `coach` makes sure you don't wreck it after moving in.
...
`vibe-coding-scaffold` 是一个 Python 项目脚手架技能，用固定的工具链（uv + ruff + mypy + ty + pytest + coverage + prek）标准化本地开发环境，消除决策疲劳。

支持四种项目类型：library（PyPI 包）、web-api（FastAPI 服务）、data-pipeline（数据管道）、cli（命令行工具）。

触发方式：`/vibe-coding-scaffold` 或自然语言描述（如"初始化一个 FastAPI 项目"）。
