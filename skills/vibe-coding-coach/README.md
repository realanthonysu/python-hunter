...
> Python-specific software engineering coach for Vibe Coding — because "running ≠ done".
...
This is the Python-specialized edition of [vibe-coding-guardian](https://github.com/realanthonysu/vibe-coding-guardian), mapping the universal iron rules to Python's concrete tools and practices.
...
| Type check (deep) | **mypy** | `mypy .` | Strict mode recommended, industry standard |
| Type check (fast) | **ty** (optional, Beta) | `ty check .` | Astral's Rust-based checker, 10-60x faster. Beta (~53% typing spec conformance), recommended as mypy complement |
| Testing | **pytest** | `pytest` | De facto standard |
| Coverage | **coverage** | `coverage run -m pytest` | Test completeness measurement |
| Security scan | **bandit** + **semgrep** | `bandit -r src/` + `semgrep --config=auto` | bandit for Python-specific SAST, semgrep for OWASP Top 10 (recommended combo) |
...
cp -r vibe-coding-coach .claude/skills/
```
...
This skill follows the [Agent Skills specification](https://agentskills.io/specification). Place the `vibe-coding-coach/` folder in your agent's skill directory.
...
└── references/
    ├── architecture.md             # Architecture coach (on-demand)
