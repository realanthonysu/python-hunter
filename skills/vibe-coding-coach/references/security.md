...
# ❌ 不受信模型文件 → 任意代码执行
import torch
model = torch.load("untrusted_model.pt")  # pickle 反序列化！
import joblib
data = joblib.load("untrusted.pkl")  # 同样是 pickle

# ✅ 安全 — 使用 weights_only=True
model = torch.load("model.pt", weights_only=True)

# ✅ 更安全 — 使用 safetensors 格式
from safetensors.torch import load_file
model = load_file("model.safetensors")
```

**规则：`torch.load()` 和 `joblib.load()` 底层使用 pickle，加载不受信文件等同于 `exec()`。ML 项目必须验证模型来源或使用安全格式。**
...
```bash
# bandit（Python 专用，AST 深度分析）
bandit -r src/
bandit -r src/ -ll  # 只显示中高风险

# semgrep（多语言通用，模式匹配，推荐与 bandit 组合使用）
semgrep --config=auto
semgrep --config=p/python           # Python 专用规则集
semgrep --config=p/owasp-top-ten    # OWASP Top 10 规则集
```
...
触发安全审查
  ...
  │
  ├─ ❷ 静态安全分析
  │     ├─ bandit -r src/（Python 专用，AST 分析）
  │     └─ semgrep --config=auto（多语言通用，模式匹配，覆盖 OWASP Top 10）
  │     └─ 存在安全问题 → 评估严重性后决定
  │
...
> **推荐组合**: bandit + semgrep 互补使用。bandit 深度分析 Python AST（如 pickle 反序列化），semgrep 覆盖更广的模式匹配（如 Django/Flask 特有问题、OWASP Top 10）。单独使用 bandit 会遗漏跨语言和框架特定的安全模式。
