...
当工具报告大量既有错误（非本次改动引入）时，不要静默忽略，按以下策略处理：
...
```bash
# ruff：只检查 git diff 中的 Python 文件
...
```bash
# 建立 mypy 基线
mypy . --no-error-summary 2> .mypy-baseline.txt
echo "Baseline: $(wc -l < .mypy-baseline.txt) errors"

# 后续对比
mypy . --no-error-summary 2> .mypy-current.txt
diff .mypy-baseline.txt .mypy-current.txt  # 只看差异
```

### 策略 3：渐进式修复

按模块逐步修复既有问题，每次修复作为独立提交：

```
提交 A：修复 src/core/ 的 mypy 错误（12 → 0）
提交 B：修复 src/api/ 的 mypy 错误（8 → 0）
提交 C：修复 src/utils/ 的 mypy 错误（5 → 0）
```

### 状态记录

当无法立即修复时，记录 SKIPPED 状态 + 理由：

```
SKIPPED: mypy 严格检查 — 项目既有 200+ 历史错误，本次改动只触及 2 个文件。
已用 git diff 限定范围检查通过。计划：下迭代建立基线并渐进修复。
```

**原则**：不完美验证 > 不验证。但 SKIPPED 必须有还债计划，不能永久跳过。
